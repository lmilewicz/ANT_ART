...
def density(u, U):
    U_stdDev = np.std(U)
    x = np.linalg.norm(u - U, axis=1) - U_stdDev
    #x_1 = np.sqrt(np.sum((u-U)**2)) - U_stdDev
    #x_2 = distance.cdist([u], U) - U_stdDev
    return len(x[x<=0])
...
def densityZ(z_ij, U_i, U_j):
    ...
    x_i = np.linalg.norm(z_ij - U_i, axis=1) - U_ij_stdDev
    x_j = np.linalg.norm(z_ij - U_j, axis=1) - U_ij_stdDev
